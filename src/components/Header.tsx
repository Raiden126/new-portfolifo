...
const Header = () => {
  ...
  return (
    <nav className={`bg-gradient-to-r from-[#5ae79a] from-10% via-[#42fde7] via-70% to-[#2cddd4] h-20 w-full flex fixed justify-between items-center px-4 sm:px-8 md:px-14 transition-all duration-300 ${isScrolled ? 'shadow-lg bg-gradient-to-r from-[#5ae79a] from-10% via-[#42fde7] via-70% to-[#2cddd4]' : ''}`}>
      <div>
        ...
        {socialIcons.map((item, index) => (
          <li className="text-xl" key={index}>
            <a href={item.url} target="_blank" rel="noopener noreferrer">
              <FontAwesomeIcon icon={item.icon} className='sm:text-xl md:text-2xl text-2xl'/>
            </a>
          </li>
        ))}
      </ul>
      <div>
        <FontAwesomeIcon icon={faBars} className='text-3xl cursor-pointer' onClick={handleMenu} />
        {menuPreview && (
          <div className='fixed top-0 left-0 w-full h-full bg-black bg-opacity-75 flex flex-col items-center justify-center animate-fadeIn'>
            ...
            <ul className="text-white text-2xl space-y-4 flex flex-col items-center">
              {navMenuItems.map((item, index) => (
                <li key={index}>
                  <a href={item.route} className='font-mono font-thin hover:underline'>
                    {item.name}
                  </a>
                </li>
              ))}
